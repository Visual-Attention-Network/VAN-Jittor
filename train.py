...
from jimm import resnext101_32x8d, resnet50, tf_efficientnet_b4_ns, van_base, van_small
jt.flags.use_cuda = 1
import torch
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_info', type=str, default='Van_base')
    ...
    # model = van_small(pretrained=False, num_classes=options['num_classes'])
    model = van_base(pretrained=False, num_classes=options['num_classes'])

    """
    van_base模型预训练下载链接：https://cloud.tsinghua.edu.cn/f/58e7acceaf334ecdba89/?dl=1
    使用前先使用torch转存pth文件，如下：
    model = torch.load('van.pth',map_location=torch.device('cpu'))
    torch.save(model['state_dict'],'van_base.pth')
    """
    model.load('van_base.pth')
